...
enum OllamaError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .http(let code):
            // The status number is the server's, not copy — it is formatted
            // without grouping so 404 never comes back as "4,04".
            return L10n.t("Ollama returned HTTP \(NumberCopy.integer(code)). Check the server address and configuration.")
        }
    }
}
