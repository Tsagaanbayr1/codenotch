        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if metrics.historyLoaded {
                Text(metrics.ledger.isEmpty
                     ...
                     : L10n.t("Today: \(NumberCopy.integer(today.requests)) requests · \(LimitWindow.compact(today.inputTokens)) tokens in · \(LimitWindow.compact(today.outputTokens)) out, across \(NumberCopy.integer(metrics.ledger.instances.count)) model(s) with history."))
                    .foregroundStyle(.secondary)
            } else {
